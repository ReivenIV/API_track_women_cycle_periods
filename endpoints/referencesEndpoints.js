...
module.exports = (app, db) => {
  ...
  app.get("/api/v1/activity_reference/all_data", errorHandler, async (req, res, next) => {
    try {
      let responseGet = await ReferencesModel.getAllActivitiesReferences();

      if (responseGet[0].length === 0) {
        return res.status(200).json({ msg: "no data in Database, for the moment" });
      }
    ...
    app.get("/api/v1/emotions_reference/all_data", errorHandler, async (req, res, next) => {
    try {
